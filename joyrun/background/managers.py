...
class UserInfoManager(models.Manager):
    def insert_user(self, username, password, email):#, object=None):
        self.create(
            username=username,
            password=password,
            email=email)
    ...
    def query_account(self, email, password):
        return self.get(email__exact=email, password__exact=password).username

    def query_user(self, username, password):
        return self.filter(
            username__exact=username, password__exact=password).count()
...
class TestCaseInfoManager(models.Manager):
    def insert_case(self, belong_module, **kwargs):
        ...
        self.create(
            ...
            belong_project=case_info.pop('project'),
    ...
    def insert_config(self, belong_module, **kwargs):
        ...
        self.create(
            ...
            belong_project=config_info.pop('project'),
            belong_module=belong_module,
            author=config_info.pop('author'),
            type=2,
            request=kwargs)
