...
from ..models import ModuleInfo, TestCaseInfo, TestSuite
...
def customer_pager(base_url, current_page, total_page):
    ...
    if current_page <= start_pager:
        ...
        prev = "<li><a href='%s%d/'><<</a></li>" % (base_url,
                                                    current_page - start_pager)
    pager_list.append(prev)

    for i in range(begin + start_pager, end + start_pager):
        if i == current_page:
            temp = "<li><a href='%s%d/' class='selected'>%d</a></li>" % (
                base_url, i, i)
        else:
    ...
    if current_page >= total_page:
        ...
        next = "<li><a href='%s%d/'>>></a></li>" % (base_url,
                                                    current_page + start_pager)
...
def get_pager_info(Model, filter_query, url, id, per_items=12):
    ...
    if 'project_list/' in url:
        ...
        if belong_project != 'All':
            obj = obj.filter(
                belong_project__project_name__contains=belong_project)
        ...
        total = obj.count()
        #  if 'testcase_list/' in url else obj.filter(type__exact=2)
        ...
        if url == '/project_list/':
            for model in info:
                ...
                module_count = str(
                    ModuleInfo.objects.filter(
                        belong_project__project_name__exact=pro_name).count())
                suite_count = str(
                    TestSuite.objects.filter(
                        belong_project__project_name__exact=pro_name).count())
                test_count = str(
                    TestCaseInfo.objects.filter(
                        belong_project__exact=pro_name, type__exact=1).count())
                config_count = str(
                    TestCaseInfo.objects.filter(
                        belong_project__exact=pro_name, type__exact=2).count())
                sum.setdefault(
            ...
            for model in info:
                ...
                test_count = str(
                    len(
                        eval(
                            TestSuite.objects.get(
                                suite_name=suite_name,
                                belong_project__project_name=project_name).
                            include)))
